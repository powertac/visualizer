...
public class CustomerService implements TimeslotCompleteActivation, Recyclable
{
  ...
  public List<TariffTransaction> getTariffTransactions (int index)
  {
    if (index >= customerList.size()) {
      return new ArrayList<TariffTransaction>();
    }
    return customerList.get(index).getCustomerModel().getTariffTransactions();
  }

  public int getCustomerType (String name)
  {
    for (int i = 0; i < customerList.size(); i++) {
      if (customerList.get(i).getCustomerInfo().getName().equals(name)) {
        return i;
      }
    }
    return 0;
